...
import subprocess
dispatcher_config_file_name = "dispatcher_case7.ini"
import pathlib
def run_worker(i_worker = 0):
    ...
    args_file1 = ["python",'yappi_main_wrapper.py', 
                  ...
                  r"demo_case1_wordcount_worker.main", f"worker{12345+i_worker}",
                  "--port", str(12345 + i_worker), "--management-port", str(22345+i_worker), "--config", 'worker.ini']
...
def run_dispatcher(n_worker):
    ...
    args_file2 = ["python",'yappi_main_wrapper.py', 
                  ...
                  r"demo_case7_dispatcher.main", 'dispatcher',"--management-port", "18000", 
                  "--config", dispatcher_config_file_name, '--n-worker', f"{n_worker}"]
...
if __name__ == "__main__":
    ...
    dispatcher_parser.read(dispatcher_config_file_name)
